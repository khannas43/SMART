-- Query to get family members based on address matching (what backend uses when cache is empty)
-- This shows the relationships that are dynamically generated by the backend's address matching logic
...
SELECT 
    ...
    EXTRACT(YEAR FROM AGE(seed_citizen.date_of_birth)) AS seed_age,
    ...
    EXTRACT(YEAR FROM AGE(related_citizen.date_of_birth)) AS member_age,
    CASE 
