...
def check_config():
    ...
    if os.path.exists(use_case_config_path):
        ...
        checker_config = use_case_config.get('eligibility_checker', {})
        print(f"      Guest mode enabled: {checker_config.get('guest_mode_enabled', False)}")
        print(f"      Anonymous checks allowed: {checker_config.get('anonymous_checks_allowed', False)}")
        
        recommendation_config = use_case_config.get('recommendation', {})
        print(f"      Top recommendations count: {recommendation_config.get('top_recommendations_count', 5)}")
    else:
