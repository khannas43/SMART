...
def check_config():
    ...
    if os.path.exists(use_case_config_path):
        ...
        detection_enabled = use_case_config.get('detection', {}).get('enable_detection', False)
        print(f"      Detection enabled: {detection_enabled}")
        
        rule_enabled = use_case_config.get('rule_detection', {}).get('recheck_eligibility_rules', False)
        ml_enabled = use_case_config.get('ml_detection', {}).get('anomaly_score_threshold', 0)
        print(f"      Rule-based detection: {rule_enabled}")
        print(f"      ML anomaly detection: {ml_enabled > 0}")
    else:
        print("   ⚠️  use_case_config.yaml not found")
    
    # 5. Check detection configuration in database
    print("\n5. Checking detection configuration in database...")
    try:
        connector = DBConnector(
            host=db_info['host'],
            port=db_info['port'],
        ...
        cursor.execute("""
            SELECT COUNT(*) 
            FROM detection.detection_config 
            WHERE is_active = TRUE
        """)
        config_count = cursor.fetchone()[0]
        print(f"   ✅ Found {config_count} active configuration entries")
        ...
        print(f"   ✅ Found {rules_count} active exclusion rules")
        
        cursor.close()
        connector.disconnect()
    except Exception as e:
        print(f"   ⚠️  Could not check database configuration: {str(e)}")
