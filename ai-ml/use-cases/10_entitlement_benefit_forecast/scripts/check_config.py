...
def check_config():
    ...
    if os.path.exists(use_case_config_path):
        ...
        forecast_config = use_case_config.get('forecast', {})
        print(f"      Default horizon: {forecast_config.get('default_horizon_months', 12)} months")
        print(f"      Supported horizons: {forecast_config.get('supported_horizons', [])}")
        
        baseline_config = use_case_config.get('baseline_forecast', {})
        print(f"      Baseline forecast enabled: {baseline_config.get('enable_baseline', False)}")
        
        scenario_config = use_case_config.get('scenario_forecast', {})
        print(f"      Scenario forecast enabled: {scenario_config.get('enable_scenarios', False)}")
    else:
